...
double t1()
{
	...
	for (int d = 2; d <= M; ++d) {
		for (int j = 1; j <= N; ++j) {
			for (int i = 0; i < link[j].size(); ++i) {	/* 최대 3개 */
				for (int k = 0; k < link[j].size(); ++k) { /* 최대 3개 */
					if (i != k) {
		...
		for (int j = 1; j <= N; ++j) {
			if (ali_p[tomorrow].val[j][0] > 0.0 && noa_p[tomorrow].val[j][0] > 0.0) {
				for (int k = 1; k <= N; ++k) {
					...
					double np = 0.0;
					for (int z = 1; z <= N; ++z) {
						if (z != k && noa_p[tomorrow].val[j][z])
							np += noa_p[tomorrow].val[j][z];
					}
...
int main()
{
	freopen("c:\\temp\\sample2.txt", "r", stdin);
